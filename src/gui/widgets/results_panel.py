...
class ResultsPanel(QWidget):
    ...
    def init_ui(self):
        # Layout principal de dos columnas (vídeo a la izquierda, datos a la derecha)
        main_layout = QHBoxLayout(self)

        # --- Columna Izquierda ---
        self.video_player = VideoPlayerWidget(self)
        main_layout.addWidget(self.video_player, 2) # Damos más espacio al vídeo

        # --- Columna Derecha ---
        right_column_widget = QWidget()
        right_column_layout = QVBoxLayout(right_column_widget)

        # Sección superior con Repeticiones y Estado
        ...
        # Creamos los checkboxes a partir del CURVE_MAP del plot_widget para que sea escalable
        ...
        self.visibility_group.hide() # Lo ocultamos hasta que haya datos

        # Sección de fallos detectados
        faults_label = QLabel("Fallos Detectados:")
        font = faults_label.font(); font.setBold(True); faults_label.setFont(font)
        right_column_layout.addWidget(faults_label)
        self.fault_list = QListWidget(self)
        right_column_layout.addWidget(self.fault_list)

        main_layout.addWidget(right_column_widget, 1) # Damos menos espacio a los datos
    ...
    def on_frame_selected(self, frame_index: int):
        ...
        if self.video_player.media_player.isSeekable():
            ...
            logger.info(f"SEÑAL RECIBIDA: Saltando al frame {frame_index} ({position_ms:.0f} ms).")
    ...
    def update_results(self, results):
        ...
        # Lógica para mostrar los checkboxes correspondientes a las curvas con datos
